...
public class MagicSpells extends JavaPlugin {
    ...
    Map<EntityType, String> entityNames;
    int globalCooldown;
    ...
    Map<ItemStack, Integer> manaPotions;
    ...
    Map<String, Spell> spellNames; // map configured names to spells
    List<Spell> spellsOrdered; // spells in loaded order
    Map<String, Spellbook> spellbooks; // player spellbooks
    Map<String, Spell> incantations; // map incantation strings to spells
    // container vars
    ManaHandler mana;
    Map<Player, Long> manaPotionCooldowns;
     ...
     */
    public static Spell getSpellByInternalName(final String spellName) {
     ...
     * @return the Spell found, or null if no spell with that name was found
     */
    public static Spell getSpellByInGameName(final String spellName) {
    ...
    public static Spellbook getSpellbook(final Player player) {
     ...
     */
    public static boolean ignoreCastItemDurability(final int type) {
     ...
     */
    public static void setManaHandler(final ManaHandler handler) {
        plugin.mana.turnOff();
        plugin.mana = handler;
    }
    ...
    public static void setItemNameResolver(final ItemNameResolver resolver) {
     ...
     */
    public static String formatMessage(final String message, final String... replacements) {
        if(message == null) {
            return null;
        }
     ...
     */
    public static void sendMessageAndFormat(final Player player, final String message, final String... replacements) {
     ...
     * @param message the message to send
     */
    public static void sendMessage(String message, final Player player, final String[] args) {
        if(message != null && !message.isEmpty()) {
            // do var replacements
            message = doArgumentAndVariableSubstitution(message, player, args);
            // send messages
            final String[] msgs = message.replaceAll("&([0-9a-fk-or])", "\u00A7$1").split("\n");
    ...
    public static String doSubjectVariableReplacements(final Player player, String string) {
        if(string != null && plugin.variableManager != null && string.contains("%var")) {
            final Matcher matcher = chatVarMatchPattern.matcher(string);
            while(matcher.find()) {
                final String varText = matcher.group();
                final String[] varData = varText.substring(5, varText.length() - 1).split(":");
                final double val = plugin.variableManager.getValue(varData[0], player);
                final String sval = varData.length == 1 ? Util.getStringNumber(val, -1) : Util.getStringNumber(val, Integer.parseInt(varData[1]));
    ...
    public static String doVariableReplacements(final Player player, String string) {
        string = doSubjectVariableReplacements(player, string);
        if(string != null && plugin.variableManager != null && string.contains("%playervar")) {
            final Matcher matcher = chatPlayerVarMatchPattern.matcher(string);
            while(matcher.find()) {
                final String varText = matcher.group();
                final String[] varData = varText.substring(11, varText.length() - 1).split(":");
                final String variableOwnerName = varData[0];
                final double val = plugin.variableManager.getValue(varData[1], variableOwnerName);
                final String sval = varData.length == 2 ? Util.getStringNumber(val, -1) : Util.getStringNumber(val, Integer.parseInt(varData[2]));
                string = string.replace(varText, sval);
            }
        }
        return string;
    ...
    public static String doArgumentSubstitution(String string, final String[] args) {
        if(string != null && string.contains("%arg")) {
            final Matcher matcher = argumentSubstitutionPattern.matcher(string);
            while(matcher.find()) {
                final String argText = matcher.group();
                final String[] argData = argText.substring(5, argText.length() - 1).split(":");
                final int argIndex = Integer.parseInt(argData[0]) - 1;
    ...
    public static String doArgumentAndVariableSubstitution(final String string, final Player player, final String[] args) {
    ...
    public static int scheduleDelayedTask(final Runnable task, final int delay) {
    ...
    public static int scheduleRepeatingTask(final Runnable task, final int delay, final int interval) {
    ...
    public static void handleException(final Exception ex) {
        if(plugin.enableErrorLogging) {
            ...
            try {
                final File folder = new File(plugin.getDataFolder(), "errors");
                if(!folder.exists()) {
    ...
    static void profilingReport() {
        if(plugin.profilingTotalTime != null && plugin.profilingRuns != null) {
            ...
            try {
                ...
                for(final String key : plugin.profilingTotalTime.keySet()) {
                    final long time = plugin.profilingTotalTime.get(key);
                    final int runs = plugin.profilingRuns.get(key);
                    totalTime += time;
    ...
    public static void addProfile(final String key, final long time) {
        if(plugin.enableProfiling) {
     ...
     */
    public static boolean teachSpell(final Player player, final String spellName) {
        ...
        final Spellbook spellbook = getSpellbook(player);
        
        if(spellbook == null || spellbook.hasSpell(spell) || !spellbook.canLearn(spell)) {
            return false;
        } else {
            // call event
            final SpellLearnEvent event = new SpellLearnEvent(spell, player, LearnSource.OTHER, null);
    ...
    void load() {
        ...
        if(config.contains("general.entity-names")) {
            final Set<String> keys = config.getSection("general.entity-names").getKeys(false);
            for(final String key : keys) {
                final EntityType entityType = Util.getEntityType(key);
                if(entityType != null) {
        ...
        final boolean opsIgnoreReagents = config.getBoolean("general.ops-ignore-reagents", true);
        final boolean opsIgnoreCooldowns = config.getBoolean("general.ops-ignore-cooldowns", true);
        final boolean opsIgnoreCastTimes = config.getBoolean("general.ops-ignore-cast-times", true);
        ...
        final Map<String, Boolean> permGrantChildren = new HashMap<>();
        final Map<String, Boolean> permLearnChildren = new HashMap<>();
        final Map<String, Boolean> permCastChildren = new HashMap<>();
        final Map<String, Boolean> permTeachChildren = new HashMap<>();
        ...
        if(config.contains("general.predefined-items")) {
            final Set<String> predefinedItems = config.getKeys("general.predefined-items");
            if(predefinedItems != null) {
                for(final String key : predefinedItems) {
                    if(config.isString("general.predefined-items." + key)) {
                        final String s = config.getString("general.predefined-items." + key, null);
                        if(s != null) {
                            final ItemStack is = Util.getItemStackFromString(s);
                            if(is != null) {
                                Util.predefinedItems.put(key, is);
                            } else {
                                error("Invalid predefined item: " + key + ": " + s);
                            }
                        }
                    } else if(config.isSection("general.predefined-items." + key)) {
                        final ConfigurationSection s = config.getSection("general.predefined-items." + key);
                        if(s != null) {
                            final ItemStack is = Util.getItemStackFromConfig(s);
                            if(is != null) {
                                Util.predefinedItems.put(key, is);
                            } else {
                                error("Invalid predefined item: " + key + ": (section)");
                            }
                        }
                    } else {
                        error("Invalid predefined item: " + key);
                    }
                }
            }
        }
        ...
        final Map<String, Boolean> advancedPermChildren = new HashMap<>();
        ...
        for(final Spell spell : spells.values()) {
            ...
            final List<String> incs = spell.getIncantations();
        ...
        final PassiveManager passiveManager = PassiveSpell.getManager();
        ...
        if(cooldownsPersistThroughReload) {
            final File file = new File(getDataFolder(), "cooldowns.txt");
                ...
                try {
                    ...
                    while(scanner.hasNext()) {
                        ...
                        if(!line.isEmpty()) {
                            ...
                            final long cooldown = Long.parseLong(data[2]);
                            if(cooldown > System.currentTimeMillis()) {
                                final Spell spell = getSpellByInternalName(data[0]);
                                if(spell != null) {
        ...
        if(enableManaBars) {
            ...
            // load mana potions
            final List<String> manaPots = config.getStringList("mana.mana-potions", null);
            if(manaPots != null && !manaPots.isEmpty()) {
                manaPotions = new LinkedHashMap<>();
                for(final String manaPot : manaPots) {
                    final String[] data = manaPot.split(" ");
                    if(data.length == 2 && RegexUtil.matches(RegexUtil.SIMPLE_INT_PATTERN, data[1])) {
                        final ItemStack item = Util.getItemStackFromString(data[0]);
                        if(item != null) {
                            manaPotions.put(item, Integer.parseInt(data[1]));
                        } else {
                            error("Invalid mana potion: " + manaPot);
                        }
                    } else {
                        error("Invalid mana potion: " + manaPot);
                    }
        ...
        final RightClickListener rightClickListener = new RightClickListener(this);
        if(rightClickListener.hasRightClickCastItems()) {
            registerEvents(rightClickListener);
        }
        final ConsumeListener consumeListener = new ConsumeListener(this);
        ...
        // register commands
        final CommandExecutor exec = new CastCommand(this, config.getBoolean("general.enable-tab-completion", true));
    ...
    private void loadSpells(final MagicConfig config, final PluginManager pm, final Map<String, Boolean> permGrantChildren, final Map<String, Boolean> permLearnChildren, final Map<String, Boolean> permCastChildren, final Map<String, Boolean> permTeachChildren) {
        // load spells from plugin folder
        final List<File> jarList = new ArrayList<>();
        for(final File file : getDataFolder().listFiles()) {
        ...
        // create class loader
        final URL[] urls = new URL[jarList.size() + 1];
        ...
        final Set<String> spellKeys = config.getSpellKeys();
        if(spellKeys == null) {
            return;
        }
        for(final String spellName : spellKeys) {
            if(config.getBoolean("spells." + spellName + ".enabled", true)) {
                ...
                try {
                    // load spell class
                    final Class<? extends Spell> spellClass = cl.loadClass(className).asSubclass(Spell.class);
                    final Constructor<? extends Spell> constructor = spellClass.getConstructor(MagicConfig.class, String.class);
                    constructor.setAccessible(true);
                    final Spell spell = constructor.newInstance(config, spellName);
                    spells.put(spellName.toLowerCase(), spell);
                    spellsOrdered.add(spell);
                    
                    // add permissions
                    if(!spell.isHelperSpell()) {
                        final String permName = spell.getPermissionName();
                ...
                if(elapsed > 50) {
                    getLogger().warning("LONG SPELL LOAD TIME: " + spellName + ": " + elapsed + "ms");
                }
            }
        }
    }
    
    private void addPermission(final PluginManager pm, final String perm, final PermissionDefault permDefault) {
        addPermission(pm, perm, permDefault, null, null);
    }
    
    private void addPermission(final PluginManager pm, final String perm, final PermissionDefault permDefault, final String description) {
        addPermission(pm, perm, permDefault, null, description);
    }
    
    private void addPermission(final PluginManager pm, final String perm, final PermissionDefault permDefault, final Map<String, Boolean> children) {
        addPermission(pm, perm, permDefault, children, null);
    }
    
    private void addPermission(final PluginManager pm, final String perm, final PermissionDefault permDefault, final Map<String, Boolean> children, final String description) {
    ...
    void unload() {
        ...
        if(cooldownsPersistThroughReload) {
            ...
            try {
                ...
                for(final Spell spell : spells.values()) {
                    final Map<String, Long> cooldowns = spell.getCooldowns();
                    for(final Entry<String, Long> stringLongEntry : cooldowns.entrySet()) {
                        final long cooldown = stringLongEntry.getValue();
                        if(cooldown > System.currentTimeMillis()) {
                            writer.append(spell.getInternalName()).append(String.valueOf(':')).append(stringLongEntry.getKey()).append(String.valueOf(':')).append(String.valueOf(cooldown)).append(String.valueOf('\n'));
                        }
                    }
                }
                writer.close();

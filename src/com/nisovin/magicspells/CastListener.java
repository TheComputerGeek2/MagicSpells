...
public class CastListener implements Listener {
    ...
    private void castSpell(Player player) {
        ...
        if(spell != null && spell.canCastWithItem()) {
            ...
            if(plugin.globalCooldown > 0 && !spell.ignoreGlobalCooldown) {
                if(noCastUntil.containsKey(player.getName()) && noCastUntil.get(player.getName()) > System.currentTimeMillis())
                    return;
    ...
    private void showIcon(Player player, int slot, ItemStack icon) {
        if(icon == null)
            icon = player.getInventory().getItem(plugin.spellIconSlot); //TODO find an alternative to reassigning the parameter

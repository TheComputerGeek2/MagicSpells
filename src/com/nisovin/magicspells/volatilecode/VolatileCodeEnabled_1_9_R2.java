...
public class VolatileCodeEnabled_1_9_R2 implements VolatileCodeHandle {
	...
	public VolatileCodeEnabled_1_9_R2() {
		try {
			...
			this.craftMetaSkullClass = Class.forName("org.bukkit.craftbukkit.v1_9_R2.inventory.CraftMetaSkull");
	...
	private NBTTagCompound getTag(ItemStack item) {
		if (item instanceof CraftItemStack) {
			try {
				return ((net.minecraft.server.v1_9_R2.ItemStack)this.craftItemStackHandleField.get(item)).getTag();
			} catch (Exception e) {
				// No op currently
			}
		}
	...
	private ItemStack setTag(ItemStack item, NBTTagCompound tag) {
		...
		try {
			nmsItem = (net.minecraft.server.v1_9_R2.ItemStack)this.craftItemStackHandleField.get(item);
		} catch (Exception e) {
			// No op currently
		}
		if (nmsItem == null) {
			...
			MagicSpells.scheduleDelayedTask(new Runnable() {
				public void run() {
					...
					if (!el.effects.isEmpty()) {
						c = net.minecraft.server.v1_9_R2.PotionBrewer.a(el.effects.values());
					}
	...
	public void toggleLeverOrButton(Block block) {
		...
		//net.minecraft.server.v1_9_R2.Block.getById(block.getType().getId()).interact(((CraftWorld)block.getWorld()).getHandle(), new BlockPosition(block.getX(), block.getY(), block.getZ()), null, 0, 0, 0, 0);
	}
	...
	public void pressPressurePlate(Block block) {
		...
		//w.applyPhysics(block.getX(), block.getY(), block.getZ(), net.minecraft.server.v1_9_R2.Block.getById(block.getType().getId()));
		//w.applyPhysics(block.getX(), block.getY()-1, block.getZ(), net.minecraft.server.v1_9_R2.Block.getById(block.getType().getId()));
	}
	...
	public Fireball shootSmallFireball(Player player) {
		net.minecraft.server.v1_9_R2.World w = ((CraftWorld)player.getWorld()).getHandle();
	...
	public void createFireworksExplosion(Location location, boolean flicker, boolean trail, int type, int[] colors, int[] fadeColors, int flightDuration) {
		// Create item
		net.minecraft.server.v1_9_R2.ItemStack item = new net.minecraft.server.v1_9_R2.ItemStack(Item.getById(401), 1, 0);
	...
	public void showItemCooldown(Player player, ItemStack item, int duration) {
		PacketPlayOutSetCooldown packet = new PacketPlayOutSetCooldown(Item.getById(item.getType().getId()), duration);
		((CraftPlayer)player).getHandle().playerConnection.sendPacket(packet);
	}

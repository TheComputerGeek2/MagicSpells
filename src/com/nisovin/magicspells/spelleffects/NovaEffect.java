...
public class NovaEffect extends SpellEffect {
	...
	private List<MagicMaterial> mats;
	...
	public Runnable playEffectLocation(Location location) {
		boolean useMultipleMaterials = mats != null && mats.size() > 0;
		if (mat == null && !useMultipleMaterials) return null;
		...
		// Start animation
		if (!circleShape) {
			if (!useMultipleMaterials) new NovaAnimationSquare(nearby, location.getBlock(), mat, radius, novaTickInterval, expandingRadiusChange);
			else new NovaAnimationSquare(nearby, location.getBlock(), mats, radius, novaTickInterval, expandingRadiusChange);
		} else {
			if (!useMultipleMaterials) new NovaAnimationCircle(nearby, location.getBlock(), mat, radius, novaTickInterval, expandingRadiusChange);
	...
	private class NovaAnimationSquare extends SpellAnimation {
		...
		MagicMaterial getRandomMaterial() {
			if (matNovas == null || matNovas.size() == 0) return matNova;

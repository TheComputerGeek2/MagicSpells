...
import com.nisovin.magicjutsus.util.VariableMod.VariableOwner;
...
public enum ModifierType {
	...
	REQUIRED(false, false, false, false, "required", "require") {
		...
		@Override
		public boolean apply(JutsuCastEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
		...
		@Override
		public boolean apply(JutsuTargetEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (!check) event.setCancelled(true);
			return check;
		}

		@Override
		public boolean apply(JutsuTargetLocationEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (!check) event.setCancelled(true);
			return check;
		}

		@Override
		public boolean apply(MagicJutsusGenericPlayerEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (!check) event.setCancelled(true);
			return check;
		}
	...
	CAST(true, false, false, false, "cast") {
		...
		public boolean apply(JutsuCastEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (check) {
				Jutsu jutsu = MagicJutsus.getJutsuByInternalName(modifierVar);
				if (jutsu != null) jutsu.cast(event.getCaster(), event.getPower(), event.getJutsuArgs());
			}
			return true;
		}
		...
		public boolean apply(ManaChangeEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (check) {
				Jutsu jutsu = MagicJutsus.getJutsuByInternalName(modifierVar);
				if (jutsu != null) jutsu.cast(event.getPlayer(), 1F, null);
			}
			return true;
		}
		...
		public boolean apply(JutsuTargetEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (check) {
				Jutsu jutsu = MagicJutsus.getJutsuByInternalName(modifierVar);
				if (jutsu != null) jutsu.cast(event.getCaster(), 1F, null);
			}
			return true;
		}
		...
		public boolean apply(JutsuTargetLocationEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (check) {
				Jutsu jutsu = MagicJutsus.getJutsuByInternalName(modifierVar);
				if (jutsu instanceof TargetedLocationJutsu) ((TargetedLocationJutsu) jutsu).castAtLocation(event.getCaster(), event.getTargetLocation(), 1F);
			}
			return true;
		}
	...
	CAST_INSTEAD(true, false, false, false, "castinstead") {
		...
		public boolean apply(JutsuTargetEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (check) {
				Jutsu jutsu = MagicJutsus.getJutsuByInternalName(modifierVar);
				if (jutsu != null) {
					if (jutsu instanceof TargetedEntityJutsu) ((TargetedEntityJutsu) jutsu).castAtEntity(event.getCaster(), event.getTarget(), 1F);
					else jutsu.castJutsu(event.getCaster(), JutsuCastState.NORMAL, 1F, null);
				}
			}
			return true;
		}
		...
		public boolean apply(JutsuTargetLocationEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData) {
			if (check) {
				Jutsu jutsu = MagicJutsus.getJutsuByInternalName(modifierVar);
				if (jutsu instanceof TargetedLocationJutsu) {
					((TargetedLocationJutsu) jutsu).castAtLocation(event.getCaster(), event.getTargetLocation(), 1F);
					event.setCancelled(true);
				}
			}
			return true;
	...
	VARIABLE_MODIFY(false, false, false, true, "variable") {
		...
		private void modifyVariable(String variableName, VariableOwner modifiedVariableOwner, Player caster, Player targetPlayer, VariableMod.Operation op, double amount) {
			...
			switch (op) {
			case SET:
				MagicJutsus.getVariableManager().set(variableName, varToModifiyOwnerPlayer, amount);
				break;
			case ADD:
				MagicJutsus.getVariableManager().modify(variableName, varToModifiyOwnerPlayer, amount);
				break;
			case MULTIPLY:
				MagicJutsus.getVariableManager().multiplyBy(variableName, varToModifiyOwnerPlayer, amount);
				break;
			case DIVIDE:
				MagicJutsus.getVariableManager().divideBy(variableName, varToModifiyOwnerPlayer, amount);
				break;
			}
		}
	...
	STRING(false, false, false, true, "string") {
		...
		public Object buildCustomActionData(String text) {
			...
			Variable variable = MagicJutsus.getVariableManager().getVariable(splits[0]);
	...
	public abstract boolean apply(JutsuCastEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData);
	...
	public abstract boolean apply(JutsuTargetEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData);
	public abstract boolean apply(JutsuTargetLocationEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData);
	public abstract boolean apply(MagicJutsusGenericPlayerEvent event, boolean check, String modifierVar, float modifierVarFloat, int modifierVarInt, Object customData);

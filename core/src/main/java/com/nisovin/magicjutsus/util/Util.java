...
import com.nisovin.magicjutsus.util.itemreader.*;
...
import com.nisovin.magicjutsus.util.handlers.PotionEffectHandler;
import com.nisovin.magicjutsus.materials.ItemNameResolver.ItemTypeAndData;
...
public class Util {
	...
	public static ItemStack getItemStackFromString(String string) {
		try {
			...
			ItemTypeAndData itemTypeAndData = MagicJutsus.getItemNameResolver().resolve(s);
			...
			if (name != null || lore != null || color >= 0) {
				try {
					...
					MagicJutsus.error("Failed to process item meta for item: " + s);
				}
			}
			if (enchants != null) {
				if (!enchants.isEmpty()) {
	...
	public static ItemStack getItemStackFromConfig(ConfigurationSection config) {
		try {
			...
			// Basic item
			MagicMaterial material = MagicJutsus.getItemNameResolver().resolveItem(config.getString("type"));
			...
			if (config.contains("enchants") && config.isList("enchants")) {
				...
				for (String enchant : enchants) {
					...
					if (e == null) MagicJutsus.error('\'' + data[0] + "' could not be connected to an enchantment");
					if (e != null) {
						int level = 0;
						if (data.length > 1) {
							try {
			...
			/*if (config.contains("flower") && item.getType() == Material.FLOWER_POT && meta instanceof BlockStateMeta) {
				MagicMaterial flower = MagicJutsus.getItemNameResolver().resolveBlock(config.getString("flower"));
			...
			// Hide tooltip
			if (config.getBoolean("hide-tooltip", MagicJutsus.hidePredefinedItemTooltips())) {
			...
			// Attributes
			AttributeManager attributeManager = MagicJutsus.getAttributeManager();

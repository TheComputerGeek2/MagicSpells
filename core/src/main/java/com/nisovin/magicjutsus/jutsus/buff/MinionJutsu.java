...
public class MinionJutsu extends BuffJutsu {
	...
	private String spawnJutsuName;
	private String deathJutsuName;
	private String attackJutsuName;
	private Ninjutsu spawnJutsu;
	private Ninjutsu deathJutsu;
	...
	public MinionJutsu(MagicConfig config, String jutsuName) {
		...
		if (potionEffectList != null && !potionEffectList.isEmpty()) {
			...
			for (String potion : potionEffectList) {
				...
				try {
					...
					MagicJutsus.error("MinionJutsu '" + internalName + "' has an invalid potion effect string " + potion);
				}
			}
		}
		...
		spawnJutsuName = getConfigString("jutsu-on-spawn", "");
		attackJutsuName = getConfigString("jutsu-on-attack", "");
		deathJutsuName = getConfigString("jutsu-on-death", "");
	...
	public void initialize() {
		...
		spawnJutsu = new Ninjutsu(spawnJutsuName);
		if (!spawnJutsu.process() && !spawnJutsuName.isEmpty()) {
			MagicJutsus.error("MinionJutsu '" + internalName + "' has an invalid jutsu-on-spawn defined!");
			spawnJutsu = null;
		}

		attackJutsu = new Ninjutsu(attackJutsuName);
		if (!attackJutsu.process() && !attackJutsuName.isEmpty()) {
			MagicJutsus.error("MinionJutsu '" + internalName + "' has an invalid jutsu-on-attack defined!");
			attackJutsu = null;
		}

		deathJutsu = new Ninjutsu(deathJutsuName);
		if (!deathJutsu.process() && !deathJutsuName.isEmpty()) {
			MagicJutsus.error("MinionJutsu '" + internalName + "' has an invalid jutsu-on-death defined!");
			deathJutsu = null;
		}
	}
	...
	public boolean castBuff(LivingEntity entity, float power, String[] args) {
		...
		if (!(minion instanceof Creature)) {
			minion.remove();
			MagicJutsus.error("MinionJutsu '" + internalName + "' Can only summon creatures!");
			return false;
		}
		...
		if (spawnJutsu != null) {
			if (spawnJutsu.isTargetedLocationJutsu()) {
				spawnJutsu.castAtLocation(player, minion.getLocation(), power);
			} else if (spawnJutsu.isTargetedEntityFromLocationJutsu()) {
				spawnJutsu.castAtEntityFromLocation(player, minion.getLocation(), minion, power);
			} else if (spawnJutsu.isTargetedEntityJutsu()) {
				spawnJutsu.castAtEntity(player, minion, power);
			}
		}
		...
		// Apply attributes
		if (attributes != null) MagicJutsus.getAttributeManager().addEntityAttributes(minion, attributes);
	...
	public void onEntityTarget(EntityTargetEvent e) {
		...
		MagicJutsus.getVolatileCodeHandler().setTarget(minion, target);
	}
	...
	public void onEntityDamage(EntityDamageByEntityEvent e) {
		...
		if (entity instanceof Player && isActive((Player) entity)) {
			...
			if (minions.get(pl.getUniqueId()).equals(damager)) {
				...
				MagicJutsus.getVolatileCodeHandler().setTarget(minions.get(pl.getUniqueId()), null);
				e.setCancelled(true);
				return;
			}
			...
			MagicJutsus.getVolatileCodeHandler().setTarget(minions.get(pl.getUniqueId()), (LivingEntity) damager);
			return;
		}
		...
		if (isMinion(entity)) {
			...
			if (((LivingEntity) entity).getHealth() - e.getFinalDamage() <= 0 && deathJutsu != null) {
				if (deathJutsu.isTargetedLocationJutsu()) {
					deathJutsu.castAtLocation(owner, minion.getLocation(), 1);
				} else if (deathJutsu.isTargetedEntityFromLocationJutsu()) {
					deathJutsu.castAtEntityFromLocation(owner, minion.getLocation(), minion, 1);
				} else if (deathJutsu.isTargetedEntityJutsu()) {
					deathJutsu.castAtEntity(owner, minion, 1);
				}
			}
			...
			if (targets.get(owner.getUniqueId()) == null || targets.get(owner.getUniqueId()).isDead() || !targets.get(owner.getUniqueId()).isValid()) {
				...
				if (minionDamager != null) {
					...
					MagicJutsus.getVolatileCodeHandler().setTarget(minion, minionDamager);
				}
			}
		}
		...
		if (damager instanceof Player) {
			...
			for (BuffJutsu buff : MagicJutsus.getBuffManager().getActiveBuffs(pl)) {
				if (!(buff instanceof MinionJutsu)) continue;
				if (entity.equals(((MinionJutsu)buff).minions.get(pl.getUniqueId()))) {
					e.setCancelled(true);
					return;
				}
			}
			...
			MagicJutsus.getVolatileCodeHandler().setTarget(minions.get(pl.getUniqueId()), (LivingEntity) entity);
		...
		if (isMinion(damager)) {
			...
			if (attackJutsu != null) {
				if (attackJutsu.isTargetedLocationJutsu()) {
					attackJutsu.castAtLocation(owner, minion.getLocation(), 1);
				} else if (attackJutsu.isTargetedEntityFromLocationJutsu()) {
					attackJutsu.castAtEntityFromLocation(owner, minion.getLocation(), (LivingEntity) entity, 1);
				} else if (attackJutsu.isTargetedEntityJutsu()) {
					attackJutsu.castAtEntity(owner, (LivingEntity) entity, 1);
				}
			}
		}
		...
		if (targets.containsValue(entity) && ((LivingEntity) entity).getHealth() - e.getFinalDamage() <= 0) {
			for (UUID id : targets.keySet()) {
				...
				targets.remove(id);
				MagicJutsus.getVolatileCodeHandler().setTarget(minions.get(id), null);

				Location loc = pl.getLocation().clone();
				loc.add(loc.getDirection().setY(0).normalize().multiply(followRange));
				MagicJutsus.getVolatileCodeHandler().creaturePathToLoc((Creature) minions.get(pl.getUniqueId()), loc, followSpeed);
			}
		}
	}

	// Owner cant damage his minion with jutsus
	@EventHandler(ignoreCancelled = true)
	public void onJutsuTarget(JutsuTargetEvent e) {
		if (!(e.getJutsu() instanceof JutsuDamageJutsu)) return;
	...
	public void onPlayerMove(PlayerMoveEvent e) {
		...
		if (pl.getLocation().distanceSquared(minion.getLocation()) > maxDistance * maxDistance || targets.get(pl.getUniqueId()) == null || !targets.containsKey(pl.getUniqueId())) {
			...
			if (targets.get(pl.getUniqueId()) != null) {
				targets.remove(pl.getUniqueId());
				MagicJutsus.getVolatileCodeHandler().setTarget(minion, null);
			}

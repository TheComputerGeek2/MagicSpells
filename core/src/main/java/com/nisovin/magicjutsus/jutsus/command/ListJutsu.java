...
public class ListJutsu extends CommandJutsu {
	...
	public ListJutsu(MagicConfig config, String jutsuName) {
		...
		jutsuFilter = JutsuFilter.fromConfig(config, "jutsus." + internalName + ".filter");
	}
	...
	public PostCastAction castJutsu(LivingEntity livingEntity, JutsuCastState state, float power, String[] args) {
		if (state == JutsuCastState.NORMAL && livingEntity instanceof Player) {
			...
			for (Jutsu jutsu : jutsubook.getJutsus()) {
				if (shouldListJutsu(jutsu, jutsubook)) {
	...
	private boolean shouldListJutsu(Jutsu jutsu, Jutsubook jutsubook) {
		if (jutsu.isHelperJutsu()) return false;
		if (onlyShowCastableJutsus && !jutsubook.canCast(jutsu)) return false;
		if (jutsusToHide != null && jutsusToHide.contains(jutsu.getInternalName())) return false;
		return jutsuFilter.check(jutsu);
	}

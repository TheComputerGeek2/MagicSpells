...
public class RightClickBlockTypeListener extends PassiveListener {
	...
	public void registerJutsu(PassiveJutsu jutsu, PassiveTrigger trigger, String var) {
		...
		for (String s : split) {
			...
			if (m == null) {
				MagicJutsus.error("Invalid type on rightclickblocktype trigger '" + var + "' on passive jutsu '" + jutsu.getInternalName() + '\'');
				continue;
			}
	...
	public void onRightClick(PlayerInteractEvent event) {
		...
		if (list != null) {
			...
			for (PassiveJutsu jutsu : list) {
				if (!PassiveListener.isCancelStateOk(jutsu, event.isCancelled())) continue;

...
public class ManaSystem extends ManaHandler {
	...
	public void initialize() {
		if (modifierList != null && !modifierList.isEmpty()) {
			MagicJutsus.debug(2, "Adding mana modifiers");
	...
	private ManaBar getManaBar(Player player) {
		...
		if (bar == null) {
			...
			MagicJutsus.debug(1, "Creating mana bar for player " + player.getName() + " with rank " + rank.getName());
	...
	public void createManaBar(final Player player) {
		...
		if (update) {
			...
			if (rank != bar.getManaRank()) {
				MagicJutsus.debug(1, "Updating mana bar for player " + player.getName() + " with rank " + rank.getName());
				bar.setRank(rank);
			}
		}
		MagicJutsus.scheduleDelayedTask(() -> showMana(player), 11);
	}
	...
	private ManaRank getRank(Player player) {
		MagicJutsus.debug(3, "Fetching mana rank for player " + player.getName() + "...");
		for (ManaRank rank : ranks) {
			MagicJutsus.debug(3, "    checking rank " + rank.getName());
			if (player.hasPermission("magicjutsus.rank." + rank.getName())) {
				MagicJutsus.debug(3, "    rank found");
				return rank;
			}
		}
		MagicJutsus.debug(3, "    no rank found");
		return defaultRank;
	}
	...
	private String getManaMessage(ManaBar bar) {
		...
		StringBuilder text = new StringBuilder(MagicJutsus.getTextColor() + bar.getPrefix() + MagicJutsus.getTextColor() + " {" + bar.getColorFull());
		int i = 0;
		for (; i < segments; i++) {
		...
		text.append(MagicJutsus.getTextColor()).append("} [").append(bar.getMana()).append('/').append(bar.getMaxMana()).append(']');
	...
	private void showManaOnExperienceBar(Player player, ManaBar bar) {
		MagicJutsus.getExpBarManager().update(player, bar.getMana(), (float) bar.getMana() / (float) bar.getMaxMana());
	}
	...
	public void turnOff() {
		...
		for (Regenerator regenerator : regenerators) {
			MagicJutsus.cancelTask(regenerator.taskId);
		}
		regenerators.clear();
	}
	...
	private class Regenerator implements Runnable {
		...
		Regenerator(ManaRank rank, int regenInterval) {
			this.rank = rank;
			taskId = MagicJutsus.scheduleRepeatingTask(this, regenInterval, regenInterval);
		}

...
public class EntityData {
	...
	public EntityData(ConfigurationSection config, boolean forceOptional) {
		...
		addOptBoolean(transformers, config, "copper-golem.waxed", CopperGolem.class, (golem, waxed) -> {
			if (waxed) golem.setOxidizing(CopperGolem.Oxidizing.waxed());
		});
		...
		ConfigData<UUID> mannequinId = getUUID(config, "mannequin.profile.uuid", null);
	...
	private ConfigData<UUID> getUUID(ConfigurationSection config, String path, UUID def) {
		ConfigData<String> uuidData = ConfigDataUtil.getString(config, path, null);

		return new ConfigData<>() {

			@Override
			public UUID get(@NotNull SpellData data) {
				String uuidString = uuidData.get(data);
				if (uuidString == null) return def;
